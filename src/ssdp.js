...
const listen = responseCallback => {
  ...
  socket.on('message', msgBuf => {
    ...
    if (
      ...
      // console.log('nodejs received message: ', msg)
      if (responseCallback) {
        responseCallback.call(null, msg)
      }
    } else if (msg.indexOf('NOTIFIY') >= 0) {
      // do nothing
    }
  })
...
const search = st => {
  const search = new Buffer(
    ['M-SEARCH * HTTP/1.1', 'HOST: 239.255.255.250:1900', 'MAN: "ssdp:discover"', 'MX: 1', `ST: ${st}`].join('\r\n')
  )
